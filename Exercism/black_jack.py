...
def value_of_ace(card_one, card_two):     
    ...
    if (card_one or card_two) == 'A':           # if one of the cards is an ace, the value of the upcoming ace must be 1 to avoid busting
        return 1
    if (card_one != 'A') and (card_two == 'A'): # makes sure that if one card is an ace, the other card is not an ace
        return 1
    if (value_of_card(card_one) + value_of_card(card_two)) > 10: # Value above 10 return 1 to avoid busting 
        return 1
    if (card_one and card_two) in numerical_values:
        if (value_of_card(card_one) + value_of_card(card_two)) < 11: # Both cards numerical and sum below 11, return 11
            return 11
    return 11
...
def is_blackjack(card_one, card_two):
    ...
    if card_one in ten_value and card_two == 'A':
        return True
    if card_two in ten_value and card_one == 'A':
        return True
    return False 
...
def can_split_pairs(card_one, card_two):
   ...
   Returns:
    ...
    """
    ten_value = {'K', 'Q', 'J', '10'}
    if (card_one and card_two in ten_value) or (card_one == card_two):
        return True
    return False
